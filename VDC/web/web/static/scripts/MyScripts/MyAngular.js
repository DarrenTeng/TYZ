...
app.controller('NavbarCtrl', ['$scope', '$location', '$http', '$cookies', 'messageService', function ($scope, $location, $http, $cookies, messageService) {
   ...
   // messageService.publish('newVM', {});
    ...
    $scope.changeStatus = function (Index) {
        ...
        $scope.selected = $scope.MyVDSList[Index];
    }
            ...
            if($scope.Guest_Os == 'Windows 10 Enterprise 32-bit'){
                ...
                if($scope.Guest_Os =='Windows 10 Professional 64-bit'){
                    ...
                    if ($scope.Guest_Os == 'Windows 10 Professional 32-bit') {
                        ...
                        if ($scope.Guest_Os == 'Windows Server 2016 Standard 64-bit') {
                            return true;
                        } else
                            if ($scope.Guest_Os == 'Windows Server 2016 DataCenter 64-bit') {
                                return true;
                    }
    }
    //initialization
    $scope.ProjectNameActive = 'true';
    $scope.GuestOsActive = 'true';
    $scope.DNSNameActive = 'true';
    $scope.DomainNarrActive = 'true';
    $scope.vCPUActive = 'true';
    $scope.LanguageActive = 'true';
    $scope.MemoryActive = 'true';
    $scope.StorageActive = 'true';
    $scope.UsernameActive = 'true';
    $scope.PasswordActive = 'true';
    $scope.ConfirmPasswordActive = 'true';
    ...
    $scope.Guest_Os_Version = '';
    $scope.CreateNewVirtualMachine = function () {
           if ($scope.ProjectName == '') {
               $scope.ProjectNameActive = 'false';
               return;
        } else
            $scope.ProjectNameActive = 'true';
           if ($scope.Guest_Os == '') {
               $scope.GuestOsActive = 'false';
               return;
           } else
               $scope.GuestOsActive = 'true';

            if ($scope.DNS_Name == '') {
                $scope.DNSNameActive = 'false';
                return;
            } else
            $scope.DNSNameActive = 'true';
            if ($scope.DomainNarr == '') {
                $scope.DomainNarrActive = 'false';
                return;
            } else
             $scope.DomainNarrActive = 'true';
             if ($scope.vCPU == '') {
                 $scope.vCPUActive = 'false';
                 return;
             } else
                 $scope.vCPUActive = 'true';
             if ($scope.getLanguageList == '') {
                 $scope.LanguageActive = 'false';
                 return;
             } else
                 $scope.LanguageActive = 'true';
              if ($scope.Memory == '') {
                  $scope.MemoryActive = 'false';
                  return;
              } else
                  $scope.MemoryActive = 'true';
              if ($scope.Storage == '') {
                  $scope.StorageActive = 'false';
                  return;
              } else
                  $scope.StorageActive = 'true';
             if ($scope.User_Name == '') {
                 $scope.UsernameActive = 'false';
                 return;
             } else
                 $scope.UsernameActive = 'true';
              if ($scope.Password == '') {
             ...
             if ($scope.Confirm_Password == '') {
                 $scope.ConfirmPasswordActive = 'false';
                 return;
             } else
                 $scope.ConfirmPasswordActive = 'true';
